locals {
  ...
  worker_groups_launch_template = [
    ...
    },
    {
      name                     = "worker-1-"
      asg_min_size             = 1
      override_instance_types  = ["m5.2xlarge", "m5d.2xlarge", "c5.2xlarge", "c4.2xlarge", "t3a.2xlarge"]
      suspended_processes      = ["AZRebalance"]
      spot_instance_pools      = 5
      spot_allocation_strategy = "lowest-price"
      kubelet_extra_args       = "--read-only-port=10255 --max-pods=60 --node-labels=node.kubernetes.io/lifecycle=spot"
      additional_userdata  = data.template_file.user_data.rendered
      tags = [
  ...
  map_roles = [
    ...
    {
      rolearn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/ROLE-PA-USW2-DEV-API"
